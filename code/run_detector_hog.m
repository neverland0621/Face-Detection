...
for i = 1:length(test_scenes)
    ...
    for scale = diff_scales
        ...
        [height,width] = size(image_scaled);                    
        for ht = 1:height - (value*value)
            for wd = 1:width - (value*value)
                image_feat = imcrop(image_scaled,[wd ht 35 35]);                       
                features_test = Hog_Compute(image_feat,value);
                
                %size(w)
                %size(features_test)
                confidence = w*features_test +b;
                if(confidence > -0.5)
